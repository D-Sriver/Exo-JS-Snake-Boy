class Grid {
    constructor(id, x, y, columns, rows, width, height) {
        function createDiv(id, borderColor, left, top, width, height) {
        ...
        this.object = createDiv(id, "red", x, y, columns * (width + 2), rows * (height + 2));
        // Création des DIVs filles
        let top = 0;
        for (let row = 0; row < rows; ++row) {
            let left = 0;
            for (let column = 0; column < columns; ++column) {
                let div = createDiv(id + "_C" + column + "R" + row, "black", left, top, width, height);
